...
# Summary/Abstract
This project seeks to evaluate whether weather conditions and population composition predict arboviral case burden across eight southeastern U.S. states. 

The primary outcome will be weekly/monthly arboviral case counts, modeled with population as an offset so results are interpreted as incidence risk. 

Key predictors will include monthly weather metrics (e.g.,`temperature` and `precipitation`), and `state`, as well as `year` and `month`.

A second aim will assess whether population proportions (based on available demographic variables) explain additional variation in cases beyond weather alone. The analysis will also test lagged weather effects (e.g., 1–3 month lags) to capture biologically realistic delays between environmental conditions and transmission, and examine whether weather–case relationships differ by state through interaction terms. I wanted to seek out this relationship as previous work I've done has seen lag of 8-12 weeks between weather events such as hurricanes and spikes in Aedes albopictus/aegypti mosquito populations, and subsequent outbreaks of arboviral diseases such as dengue and Zika in Puerto Rico.

I expect that higher temperatures and increased precipitation will be associated greatly with increased arboviral case burden, as these align with favorable breeding conditions for mosquito vectors. I also expect that rather than state population, state geography specifically along with weather data will better predict case burden. 

Background/Methods/Results/Discussion sections._
